...
- **THEN** the system SHALL create a `generationJobs` record with `totalItems: 10` and enqueue 10 actions into the pool

#### Scenario: Dimension-driven strategy enqueues single item
- **WHEN** a user starts generation with "dimension-driven" strategy
- **THEN** the system SHALL create a `generationJobs` record with `totalItems: 1` and enqueue 1 action into the pool

#### Scenario: Dedup running generation
- **WHEN** a generation job is already running for the same dataset
- **THEN** the mutation SHALL return the existing job ID without creating a new one
...
The system SHALL provide an `onQuestionGenerated` mutation as the WorkPool `onComplete` callback for Phase 1. It SHALL increment progress counters on the `generationJobs` record. When all Phase 1 items have completed (processedItems + failedItems + skippedItems >= totalItems), it SHALL transition the job to Phase 2 by: updating `phase` to `"ground-truth"`, querying all generated questions for the dataset, setting `totalItems` to the question count, resetting `processedItems`/`failedItems`/`skippedItems` to 0, and enqueueing one `assignGroundTruthForQuestion` action per question into the same pool with `onComplete: onGroundTruthAssigned`.
...
- **THEN** the callback SHALL transition to Phase 2, query all questions, and enqueue GT actions
...
The system SHALL provide an `onGroundTruthAssigned` mutation as the WorkPool `onComplete` callback for Phase 2. It SHALL increment progress counters. When all Phase 2 items complete, it SHALL finalize the job by: updating `datasets.questionCount`, setting `generationJobs.status` to `"completed"` (or `"completed_with_errors"` if any items failed), setting `completedAt`, and scheduling a fire-and-forget LangSmith sync.

#### Scenario: All GT items complete with no failures
- **WHEN** all ground truth items complete successfully
- **THEN** the job status SHALL be set to `"completed"` and dataset `questionCount` SHALL be updated

#### Scenario: Some GT items failed
...
The system SHALL provide a `cancelGeneration` mutation that sets the job status to `"canceling"` and calls `pool.cancelAll(ctx)`. Pending pool items SHALL be canceled; in-flight items SHALL complete normally. The job SHALL transition to `"canceled"` when all in-flight items finish.

#### Scenario: Cancel mid-generation
- **WHEN** a user cancels during Phase 1 with 5/10 items processed
- **THEN** pending items SHALL be canceled, in-flight items SHALL finish, and the job SHALL eventually reach `"canceled"` status
