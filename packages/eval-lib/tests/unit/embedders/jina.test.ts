...
describe("JinaEmbedder", () => {
  ...
  describe("create() HTTP wire contract", () => {
    ...
    it("POSTs to the Jina embeddings endpoint with bearer auth + task body", async () => {
      fetchSpy.mockResolvedValueOnce(
        mockFetchResponse({ data: [{ embedding: [0.1, 0.2, 0.3], index: 0 }] })
      )
      const embedder = await JinaEmbedder.create({ apiKey: "test-key" })
    ...
    it("throws a clear error when no API key is available", async () => {
      ...
      await expect(JinaEmbedder.create()).rejects.toThrow(/Jina API key/)
