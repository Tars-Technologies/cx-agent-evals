...
const mockLLMClient: LLMClient = {
  ...
  async complete(params: {
    ...
    if (extractCount > 0) {
      const existingQMatches = [...userMsg.matchAll(/\d+\.\s+(.*?\?)/g)]
...
const matchingEmbedder: Embedder = {
  ...
  embed: async (texts: readonly string[]) =>
    texts.map((text) => {
      ...
      if (
        ...
        text.toLowerCase().includes("docker") ||
  ...
  embedQuery: async (query: string) => {
    const vec = new Array(10).fill(0)
    if (
      query.toLowerCase().includes("kubernetes") ||
      query.toLowerCase().includes("pod")
    ) {
      vec[0] = 1
    } else if (
      query.toLowerCase().includes("docker") ||
...
function makeConfig(
  overrides: Partial<UnifiedGenerationConfig> = {}
): UnifiedGenerationConfig {
  return {
    totalQuestions: 4,
...
function makeContext(
  overrides: Partial<UnifiedGeneratorContext> = {}
): UnifiedGeneratorContext {
  return {
    corpus: testCorpus,
  ...
  describe("only dimensions (Scenario 3)", () => {
    ...
    const dimensions = [
      ...
      },
      {
        ...
        values: ["troubleshooting", "learning"]
      }
    ]
  ...
  describe("both dimensions and real-world questions", () => {
    ...
    it("uses mixed scenarios across documents", async () => {
      const generator = new UnifiedQuestionGenerator(
        ...
        makeContext({
          llmClient: filterAwareLLMClient,
          embedder: matchingEmbedder
        })
      )
      const result = await generator.generate()
          ...
          return JSON.stringify({
            questions: [
              ...
              },
              {
                ...
                citation:
                  "This text does not appear anywhere in any document at all and is completely made up gibberish xyz123.",
                source: "generated",
                profile: null
              }
            ]
