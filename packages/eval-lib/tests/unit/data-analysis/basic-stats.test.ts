...
describe("computeBasicStats", () => {
  it("should count total conversations", async () => {
    const stats = await computeBasicStats(
      toAsyncIterable([makeRow(), makeRow({ "Conversation ID": "2" })])
    )
  ...
  it("should count conversations with and without user messages", async () => {
    const rows = [
      ...
      makeRow({
        "Conversation ID": "2",
        "Number of messages sent by the visitor": "0"
      })
    ]
    const stats = await computeBasicStats(toAsyncIterable(rows))
  ...
  it("should compute duration stats", async () => {
    const rows = [
      ...
      makeRow({
        "Conversation ID": "2",
        "Total Conversation duration in Seconds": "200"
      }),
      makeRow({
        "Conversation ID": "3",
        "Total Conversation duration in Seconds": "300"
      })
    ]
    const stats = await computeBasicStats(toAsyncIterable(rows))
  ...
  it("should break down labels", async () => {
    const rows = [
      ...
      makeRow({
        "Conversation ID": "2",
        Labels: "campaign_mobile,language_arabic"
      })
    ]
    const stats = await computeBasicStats(toAsyncIterable(rows))
  ...
  it("should break down agents", async () => {
    const rows = [
      makeRow({
        "Agent Name": "Aya",
        "Agent Email": "aya@test.com",
        "Number of messages sent by the agent": "10"
      }),
      makeRow({
        "Conversation ID": "2",
        "Agent Name": "Aya",
        "Agent Email": "aya@test.com",
        "Number of messages sent by the agent": "5"
      })
    ]
    const stats = await computeBasicStats(toAsyncIterable(rows))
