    ...
    messages: [
      {
        id: 1,
        role: "workflow_input",
        text: "Continue in English, -No Input-, New Postpaid Plan, English,"
      },
      ...
      { id: 4, role: "human_agent", text: "Hi there!" },
      {
        id: 5,
        role: "workflow_input",
        text: "Conversation was marked resolved by Agent"
      }
    ],
    metadata: {
...
describe("isSystemMessage", () => {
  ...
  it("should identify resolution messages", () => {
    expect(isSystemMessage("Conversation was marked resolved by John")).toBe(
      true
    )
  })
  ...
  it("should identify label addition messages", () => {
    expect(
      isSystemMessage("John added campaign_mobile, language_english")
    ).toBe(true)
  })
...
describe("assembleConversation", () => {
  it("should merge LLM results with system messages in ID order", () => {
    ...
    expect(
      result.microtopics[1].exchanges[0].messages.map((m) => m.id)
    ).toEqual([3, 4])
