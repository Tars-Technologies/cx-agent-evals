...
describe("RecursiveCharacterChunker", () => {
  it("should reject overlap >= chunkSize", () => {
    expect(
      () => new RecursiveCharacterChunker({ chunkSize: 100, chunkOverlap: 100 })
    ).toThrow()
    expect(
      () => new RecursiveCharacterChunker({ chunkSize: 100, chunkOverlap: 200 })
    ).toThrow()
  })
...
describe("overlap duplication bug", () => {
  ...
  it("should produce roughly proportional chunk counts for smaller chunkSize", () => {
    const large = new RecursiveCharacterChunker({
      chunkSize: 1000,
      chunkOverlap: 200
    })
    const small = new RecursiveCharacterChunker({
      chunkSize: 400,
      chunkOverlap: 80
    })
