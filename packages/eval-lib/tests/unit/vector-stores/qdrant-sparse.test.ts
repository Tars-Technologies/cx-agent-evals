...
describe("QdrantVectorStore (sparse / named hybrid)", () => {
  ...
  it("rejects a malformed dense: null shape with the designed refusal, not a TypeError", async () => {
    fetchMock.mockResolvedValueOnce(
      okJson({
        status: "ok",
        result: { config: { params: { vectors: { dense: null } } } }
      })
    )
    await expect(
      store.add([chunk("c1")], [[1, 0, 0]], VALID_SCOPE)
  ...
  it("rejects a primitive vectors shape with the designed refusal, not a TypeError", async () => {
    fetchMock.mockResolvedValueOnce(
      okJson({
        status: "ok",
        result: { config: { params: { vectors: "corrupted" } } }
      })
    )
    await expect(
      store.add([chunk("c1")], [[1, 0, 0]], VALID_SCOPE)
