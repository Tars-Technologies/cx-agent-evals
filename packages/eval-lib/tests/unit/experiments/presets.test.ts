...
describe("createBaselineVectorRagRetriever", () => {
  ...
  it("config override: can set a custom name", () => {
    retriever = createBaselineVectorRagRetriever(
      { chunker, embedder },
      { name: "my-custom-rag" }
    )
...
describe("createHybridRetriever", () => {
  ...
  it("config has correct weights (denseWeight and sparseWeight)", () => {
    ...
    const search = HYBRID_CONFIG.search as {
      denseWeight?: number
      sparseWeight?: number
    }
...
describe("createHybridRerankedRetriever", () => {
  ...
  it('creates a retriever with name "hybrid-reranked"', () => {
    retriever = createHybridRerankedRetriever({
      chunker,
      embedder,
  ...
  it("config includes rerank refinement step", () => {
    ...
    const rerankStep = HYBRID_RERANKED_CONFIG.refinement!.find(
      (step) => step.type === "rerank"
    )
...
describe("createPresetRetriever (registry-backed factory)", () => {
  ...
  const depsWithReranker: PipelinePresetDeps = {
    ...baseDeps,
    reranker: mockReranker
  }
  const llm = mockLLM()
  const depsWithLlm: PipelinePresetDeps = { ...baseDeps, llm }
  const fullDeps: PipelinePresetDeps = {
    ...baseDeps,
    reranker: mockReranker,
    llm
  }
  ...
  it.each(
    basicPresets.map((n) => ({ name: n }))
  )("$name creates a retriever with base deps", ({ name }) => {
    const retriever = createPresetRetriever(name, baseDeps)
    expect(retriever.name).toBe(name)
  })

  it.each(
    rerankerOnlyPresets.map((n) => ({ name: n }))
  )("$name creates a retriever with reranker deps", ({ name }) => {
    const retriever = createPresetRetriever(name, depsWithReranker)
    expect(retriever.name).toBe(name)
  })

  it.each(
    llmOnlyPresets.map((n) => ({ name: n }))
  )("$name creates a retriever with llm deps", ({ name }) => {
    const retriever = createPresetRetriever(name, depsWithLlm)
    expect(retriever.name).toBe(name)
  })

  it.each(
    llmAndRerankerPresets.map((n) => ({ name: n }))
  )("$name creates a retriever with full deps", ({ name }) => {
