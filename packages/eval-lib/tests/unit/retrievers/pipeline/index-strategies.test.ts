...
describe("PipelineRetriever — contextual index strategy", () => {
  ...
  it("should respect custom contextPrompt", async () => {
    ...
    const customPrompt =
      "Custom context: {doc.content} | Chunk: {chunk.content}"
    const config: PipelineConfig = {
    ...
    const customRetriever = new PipelineRetriever(
      config,
      defaultDeps({ llm: customLlm })
    )
...
describe("PipelineRetriever — summary index strategy", () => {
  ...
  beforeEach(async () => {
    ...
    mockLlm = createMockLlm(
      "A summary of important topics covered in this passage."
    )
