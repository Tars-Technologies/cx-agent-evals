...
export async function generateFewShotQuestions(
  ...
  if (docContent.length > maxChars) {
    console.warn(
      `Document "${options?.docId ?? "unknown"}" truncated from ${docContent.length} to ${maxChars} chars`
    )
  }
...
export function distributeBudget(
  ...
  const unmatchedDocs = allDocIds.filter(
    (id) => !matchCountByDoc.has(id) || matchCountByDoc.get(id) === 0
  )
  ...
  if (remainingBudget > 0 && totalMatches > 0) {
    const matchedDocs = allDocIds.filter(
      (id) => matchCountByDoc.has(id) && matchCountByDoc.get(id)! > 0
    )
    let allocated = 0
