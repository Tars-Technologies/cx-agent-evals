...
export class DimensionDrivenStrategy implements QuestionStrategy {
  ...
  async generate(context: StrategyContext): Promise<GeneratedQuery[]> {
    ...
    console.log(
      `[DimensionDriven] Funnel: ${validCombos.length} combos after filtering`
    )
    ...
    console.log(
      `[DimensionDriven] Funnel: ${matrix.assignments.length} assignments from relevance matrix`
    )
    ...
    console.log(
      `[DimensionDriven] Funnel: ${sampled.length} sampled (target: ${this._options.totalQuestions}), deficit: ${deficit}`
    )
    if (deficit > 0) {
      console.warn(
    ...
    const docIndex = new Map(
      context.corpus.documents.map((d) => [String(d.id), d])
    )
    ...
    for (let docIdx = 0; docIdx < docEntries.length; docIdx++) {
      ...
      if (doc.content.length > maxChars) {
        console.warn(
          `Document "${docId}" truncated from ${doc.content.length} to ${maxChars} chars`
        )
      }
      ...
      const data = safeParseLLMResponse(response, {
        questions: [] as Array<{ profile_index: number; question: string }>
      })
    ...
    if (deficit > 0) {
      ...
      for (
        let dIdx = 0;
        dIdx < allDocIds.length &&
        results.length < this._options.totalQuestions;
        dIdx++
      ) {
        ...
        const needed = Math.min(
          deficitPerDoc,
          this._options.totalQuestions - results.length
        )
        if (needed <= 0) break
        ...
        const fillData = safeParseLLMResponse(fillResponse, {
          questions: [] as string[]
        })
