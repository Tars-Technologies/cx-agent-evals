...
export class GroundTruthAssigner
  implements GroundTruthAssignerInterface<GroundTruth>
{
  ...
  private async _extractExcerpts(
    ...
    if (docContent.length > maxChars) {
      console.warn(
        `Document "${docId}" truncated from ${docContent.length} to ${maxChars} chars`
      )
    }
    ...
    return (
      safeParseLLMResponse(response, { excerpts: [] as string[] }).excerpts ??
      []
    )
  }
  ...
  private _fuzzyFindSentences(
    ...
    for (const sentence of sentences) {
      ...
      if (bestScore <= threshold && bestStart !== -1) {
        ...
        while (bestStart > 0 && !/\s/.test(docContent[bestStart - 1]))
          bestStart--
        let bestEnd = bestStart + windowSize
        while (bestEnd < docContent.length && !/\s/.test(docContent[bestEnd]))
          bestEnd++
