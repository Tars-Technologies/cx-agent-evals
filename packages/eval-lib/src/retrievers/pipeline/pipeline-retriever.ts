...
} from "../../chunkers/chunker.interface.js"
...
import { assignRankScores, DenseSearchStrategy } from "./search/dense.js"
import { rrfFuseMultiple } from "./search/fusion.js"
import { HybridSearchStrategy } from "./search/hybrid.js"
import type {
  SearchStrategy,
  SearchStrategyDeps
} from "./search/strategy.interface.js"
...
export class PipelineRetriever implements Retriever {
  ...
  async init(corpus: Corpus): Promise<void> {
    ...
    switch (this._indexConfig.strategy) {
      ...
      case "contextual": {
        const contextPrompt =
          this._indexConfig.contextPrompt || DEFAULT_CONTEXT_PROMPT
      ...
      case "summary": {
        const summaryPrompt =
          this._indexConfig.summaryPrompt || DEFAULT_SUMMARY_PROMPT
        const concurrency = this._indexConfig.concurrency ?? 5
        ...
        for (const doc of corpus.documents) {
          ...
          const summarized = await mapWithConcurrency(
            rawChunks,
            async (chunk) => {
              const summary = await this._llm!.complete(
                summaryPrompt + chunk.content
              )
  ...
  async retrieve(query: string, k: number): Promise<PositionAwareChunk[]> {
    ...
    if (this._childToParentMap) {
      ...
      for (const scored of scoredResults) {
        const parent =
          this._childToParentMap.get(String(scored.chunk.id)) ?? scored.chunk
