...
export class RecursiveCharacterChunker
  implements Chunker, PositionAwareChunker
{
  ...
  chunk(text: string): string[] {
    return this._splitTextWithPositions(text, this._separators, 0).map(
      ([t]) => t
    )
  }
  ...
  private _splitTextWithPositions(
    ...
    if (text.length <= this._chunkSize) {
      ...
      return [
        [
          ...
          baseOffset + trimStart + trimmed.length
        ]
      ]
    }
    ...
    if (separator === "") {
      ...
      for (
        let i = 0;
        i < text.length;
        i += this._chunkSize - this._chunkOverlap
      ) {
        const slice = text.substring(i, i + this._chunkSize).trim()
        if (slice.length > 0) {
          const trimStart = text
            .substring(i, i + this._chunkSize)
            .indexOf(slice)
    ...
    for (const part of parts) {
      const addLen =
        currentLen === 0
          ? part.text.length
          : separator.length + part.text.length
