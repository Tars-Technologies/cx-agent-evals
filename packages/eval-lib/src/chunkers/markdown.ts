...
export class MarkdownChunker implements Chunker, PositionAwareChunker {
  ...
  chunkWithPositions(doc: Document): PositionAwareChunk[] {
    ...
    const merged = this._mergeSmallSections ? this._merge(sections) : sections
   ...
   */
  private _splitAtHeaders(text: string): Array<{ start: number; end: number }> {
    ...
    for (let i = 0; i < boundaries.length; i++) {
      ...
      const rawEnd = i < boundaries.length - 1 ? boundaries[i + 1] : text.length
