...
import {
  QuestionListPane,
  type StatusFilter
} from "@/components/retriever-detail/QuestionListPane"
...
export default function RetrieverDetailPage({
  ...
  const backHref = experiment.experimentRunId
    ? buildKbLink(
        `/retrievers?runId=${experiment.experimentRunId}&mode=experiment`,
        kbId
      )
  ...
  const cfg = experiment.retrieverConfig as {
    index?: { chunkSize?: number }
    k?: number
    search?: { embedder?: string }
  } | null
          ...
          <QuestionDetailPane
            question={
              visibleQuestions.find(
                (q) => q.resultId === effectiveSelectedId
              ) ?? null
            }
            metricNames={experiment.metricNames}
          />
