...
function sourceKey(
  ...
  return source.kind === "conversation"
    ...
    : `t:${source.transcriptId}`;
}

function memberKey(m: Member): string {
  return sourceKey(m.source);
}
...
function MinimalAnnotationPanel({
  ...
  // This panel stays mounted while the user clicks between conversations in the
  // list, so remount the editor on conversation change (via `key`) to reset its
  // local edit state for the new target.
  const conversationKey = sourceKey(conversationRef);
