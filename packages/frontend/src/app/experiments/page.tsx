...
import { buildKbLink, useKbFromUrl } from "@/lib/useKbFromUrl"
...
function ExperimentsPageContent() {
  ...
  const [experimentMode, setExperimentMode] = useState<"agent" | "retriever">(
    "agent"
  )
  ...
  const [runningExperimentIds, setRunningExperimentIds] = useState<
    Set<Id<"experiments">>
  >(new Set())
  ...
  async function handleRunExperiments() {
    ...
    const retrieverList = readyRetrievers.filter((r) =>
    ...
    for (const retriever of retrieverList) {
      try {
        ...
        setRunningExperimentIds(
          (prev) => new Set([...prev, result.experimentId])
        )
      } catch (err) {
        setError(
  ...
  async function handleRunAgentExperiment() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to start agent experiment"
      )
    }
  }
                  ...
                  <div className="border border-border rounded-lg bg-bg">
                    ...
                    <div className="p-4 space-y-4">
                      ...
                      >
                        ...
                        {selectedRetrieverIds.size > 1 &&
                          `(${selectedRetrieverIds.size})`}
                      </button>
              ...
              <div className="p-4">
                {!selectedKbId ? (
                  <p className="text-text-dim text-sm">
                    Select a knowledge base to see experiments.
                  </p>
  ...
  return (
    <div className="border border-border rounded-lg p-4 hover:border-border/80 transition-colors">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded ${statusColors[exp.status] ?? "bg-text-dim/10 text-text-dim"}`}
        >
          {exp.status}
        </span>
      </div>
      {exp.status === "running" && exp.processedQuestions != null && (
        <div className="mt-1 text-xs text-text-dim">
          {exp.phase ?? "Evaluating"}... ({exp.processedQuestions}/
          {exp.totalQuestions ?? "?"})
        </div>
      )}
