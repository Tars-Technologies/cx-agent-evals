...
export function IndexSearchNote({
  indexStrategy,
  indexConfig
}: {
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-text-dim">
            ...
            {staticSearchConfig.denseWeight != null && (
              <span>
                Dense:{" "}
                <span className="text-text-muted">
                  {staticSearchConfig.denseWeight}
                </span>
              </span>
            )}
            {staticSearchConfig.sparseWeight != null && (
              <span>
                Sparse:{" "}
                <span className="text-text-muted">
                  {staticSearchConfig.sparseWeight}
                </span>
              </span>
            )}
            {staticSearchConfig.fusionMethod != null && (
              <span>
                Fusion:{" "}
                <span className="text-text-muted">
                  {staticSearchConfig.fusionMethod}
                </span>
              </span>
            )}
            <span>
            ...
            return (
              <>
                ...
                <div className="px-3 pb-2">
                  <p className="text-[11px] text-text-dim">
                    {resultHeader} &middot; {latencyMs}ms
                  </p>
                </div>
...
export function QuerySearchTab({
  ...
  const staticSearchConfig = {
    ...
    ...(resolved.search.strategy === "hybrid"
      ? {
          denseWeight: (resolved.search as any).denseWeight as
            | number
            | undefined,
          sparseWeight: (resolved.search as any).sparseWeight as
            | number
            | undefined,
          fusionMethod: (resolved.search as any).fusionMethod as
            | string
            | undefined
        }
      : {})
  ...
  const indexConfig = (retriever.retrieverConfig?.index ?? {}) as Record<
    string,
    unknown
  >
  ...
  const rewriteQueryAction = useAction(api.kb.pipelineActions.rewriteQuery)
