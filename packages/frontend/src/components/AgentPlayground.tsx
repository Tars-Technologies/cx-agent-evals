...
export default function AgentPlayground({ agentId }: AgentPlaygroundProps) {
  ...
  const getOrCreate = useMutation(
    api.agents.orchestration.getOrCreatePlayground
  )
            ...
            return (
              <div key={msg._id} className="flex justify-start">
                <div className="max-w-[80%]">
                  <div className="text-text-dim text-[8px] mb-0.5 ml-1">
                    Agent
                  </div>
                  <div
                    className={`bg-bg-elevated border rounded-xl px-3 py-2 ${isError ? "border-red-500/30" : "border-border"}`}
                  >
                    {isStuck && !displayContent ? (
                      <p className="text-sm text-red-400">
                        Agent is not responding. Check that ANTHROPIC_API_KEY
                        (or OPENAI_API_KEY for OpenAI models) is set in Convex
