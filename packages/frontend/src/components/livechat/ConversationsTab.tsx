...
export function ConversationsTab({
  ...
  // --- State ---
  const [view, setView] = useState<"conversation" | "messageType">(
    "conversation"
  )
  const [selectedConvId, setSelectedConvId] =
    useState<Id<"livechatConversations"> | null>(null)
  const [selectedType, setSelectedType] =
    useState<MessageTypeCategory>("question")
  ...
  const [pendingTranslate, setPendingTranslate] = useState<Set<string>>(
    new Set()
  )
  ...
  const translateSingle = useMutation(
    api.livechat.orchestration.translateSingle
  )
  ...
  const effectiveClassificationStatus =
    selectedConvId && pendingClassify.has(selectedConvId)
      ? "running"
      : selectedConv?.classificationStatus
  const effectiveTranslationStatus =
    selectedConvId && pendingTranslate.has(selectedConvId)
      ? "running"
  ...
  const batchProgress = useMemo(() => {
    ...
    const statusField =
      batchOp.type === "classify" ? "classificationStatus" : "translationStatus"
    let done = 0
  ...
  const hasTranslatableMessages = useMemo(() => {
    ...
    return selectedConv.messages.some((m: { text: string }) =>
      needsTranslation(m.text)
    )
  }, [selectedConv])
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 overflow-hidden">
                <ConversationList
                  ...
                  onSelect={(id) =>
                    setSelectedConvId(id as Id<"livechatConversations">)
                  }
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {(effectiveClassificationStatus === "none" ||
                        ...
                        <button
                          onClick={() => {
                            setPendingClassify((prev) =>
                              new Set(prev).add(selectedConvId!)
                            )
                      ...
                      {hasTranslatableMessages &&
                        (effectiveTranslationStatus === "none" ||
                      ...
                      (selectedConv.messageTypes as any[]).map(
                        (mt: any, i: number) => (
                          <MessageTypeCard
                            ...
                            translatedMessages={
                              (selectedConv as any).translatedMessages
                            }
                          />
                        )
                      )
            ...
            <div className="bg-bg-elevated px-3 py-2 border-b border-border flex justify-between items-center">
              <div>
                <span className="text-accent text-xs font-semibold">
                  {(messagesByType.get(selectedType) ?? []).length}{" "}
                  {selectedType}s
                </span>
                <span className="text-text-dim text-[10px] ml-2">
              ...
              <ExportButton
                data={{
                  ...
                  items: (messagesByType.get(selectedType) ?? []).map(
                    (item) => ({
