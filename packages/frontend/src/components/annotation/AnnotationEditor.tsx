...
export function AnnotationEditor({
  ...
  // Adopt the saved annotation into local state UNTIL the user starts editing.
  // After the first local edit (dirtyRef) we stop syncing from the server —
  // otherwise the round-trip from our own debounced save (server → useQuery →
  // a new object every render) would reset the textarea and wipe characters
  // typed during the round-trip. To annotate a *different* conversation in a
  // reused editor, remount it with a React `key` (which resets dirtyRef).
  ...
  useEffect(() => {
    ...
    setSaveState(existingAnnotation ? "saved" : "idle");
  }, [existingAnnotation]);
