    ...
    <div className="px-6 py-4 space-y-5">
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <Chip
            color={
              scenario.persona.patienceLevel === "low"
                ? "red"
                : scenario.persona.patienceLevel === "high"
                  ? "green"
                  : "yellow"
            }
          >
      ...
      <section>
        ...
        <div className="text-xs text-text-dim leading-relaxed">
          <strong className="text-text">Reason for contact:</strong>{" "}
        ...
        <section>
          ...
          <ul className="space-y-1">
            {scenario.behaviorAnchors.map((a, i) => (
              <li
                key={i}
                className="text-xs text-text-dim leading-relaxed flex"
              >
          ...
          {scenario.userMessageLengthStats && (
            <div className="mt-2">
              <Chip color="gray">
                Typical length: median {scenario.userMessageLengthStats.median}w
                / p90 {scenario.userMessageLengthStats.p90}w
              </Chip>
            </div>
          )}
        </section>
      ...
      {scenario.referenceTranscript &&
      scenario.referenceTranscript.length > 0 ? (
        <section>
          <h3 className="text-[11px] text-text-dim uppercase tracking-wider mb-2">
            Source transcript ({scenario.referenceTranscript.length} messages)
          </h3>
          <div className="text-xs text-text-dim leading-relaxed">
            Open this scenario in a simulation run to compare against the
            snapshot.
          </div>
        </section>
      ) : scenario.referenceExemplars &&
        scenario.referenceExemplars.length > 0 ? (
        <section>
          <h3 className="text-[11px] text-text-dim uppercase tracking-wider mb-2">
              ...
              {scenario.referenceExemplars.slice(0, 3).map((ex, i) => (
                ...
                >
                  ...
                  {ex.messages.map((m, j) => (
                    <div
                      key={j}
                      className="text-xs text-text-dim leading-relaxed mb-0.5"
                    >
      ...
      ) : scenario.referenceMessages &&
        scenario.referenceMessages.length > 0 ? (
        <section>
          <h3 className="text-[11px] text-text-dim uppercase tracking-wider mb-2">
...
export function Chip({
  ...
  return (
    <span
      className={`px-1.5 py-0.5 text-[9px] rounded border ${colorMap[color]}`}
    >
      {children}
    </span>
  )
