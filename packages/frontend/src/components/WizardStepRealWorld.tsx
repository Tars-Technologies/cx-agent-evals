...
export function WizardStepRealWorld({
  questions,
  onChange,
  onNext,
  onSkip
}: WizardStepRealWorldProps) {
    ...
    <div className="space-y-4 animate-fade-in">
      <div>
        ...
        <p className="text-xs text-text-dim mt-1">
          Upload a CSV or paste real questions from your users. These help
          generate more realistic evaluation questions.
        </p>
      </div>
      ...
      {count > 0 && (
        <p className="text-xs text-accent">
          {count} question{count !== 1 ? "s" : ""} · will be matched to
