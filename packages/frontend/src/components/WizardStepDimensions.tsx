...
export function WizardStepDimensions({
  kbId,
  dimensions,
  onChange,
  onNext,
  onSkip,
  onBack
}: WizardStepDimensionsProps) {
  const [url, setUrl] = useState(() => {
    try {
      return localStorage.getItem(discoverUrlKey(kbId)) ?? ""
    } catch {
      return ""
    }
  })
  ...
  const handleDiscover = useCallback(async () => {
    ...
    try {
      ...
      try {
        localStorage.setItem(discoverUrlKey(kbId), url)
      } catch {}
    } catch (e: unknown) {
    ...
    <div className="space-y-4 animate-fade-in">
      <div>
        ...
        <p className="text-xs text-text-dim mt-1">
          Auto-discover user personas and question types from your product URL,
          or add manually.
        </p>
      </div>
      ...
      {dimensions.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {dimensions.map((dim, di) => (
            <div
              key={di}
              className="p-3 border border-border rounded space-y-2"
            >
              <div className="flex items-center justify-between">
                <input
                  ...
                  value={dim.name}
                  onChange={(e) =>
                    updateDimension(di, { name: e.target.value })
                  }
              ...
              <div className="flex flex-wrap gap-1">
                {dim.values.map((val, vi) => (
                  <span
                    key={vi}
                    className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-bg-secondary border border-border text-text-dim"
                  >
                    {val}
                    <button
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onNext}
            disabled={dimensions.length === 0 || dimensions.some(d => !d.name.trim() || !d.description.trim() || d.values.length < 2)}
            className="px-3 py-1.5 text-xs rounded bg-accent-dim text-accent-bright hover:bg-accent/20 transition-colors disabled:opacity-40"
          >
            Next →
          </button>
