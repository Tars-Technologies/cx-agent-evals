  ...
  useEffect(() => {
    function onMouseMove(e: MouseEvent) {
      ...
      if (leftPane.flex) {
        ...
        const rightStart =
          drag.startWidths.get(rightPane.id) ?? rightPane.defaultWidth
        ...
        const leftStart =
          drag.startWidths.get(leftPane.id) ?? leftPane.defaultWidth
  ...
  return (
    ...
    >
      {panes.map((pane, i) => {
        ...
        const nextVisibleIndex = visiblePaneIndices.findIndex(
          ({ i: vi }) => vi > i
        )
        const hasHandleAfter = nextVisibleIndex !== -1 && !pane.flex // flex pane shouldn't have a handle after it in a typical layout

        const paneWidth = pane.flex
          ? undefined
          : (widths.get(pane.id) ?? pane.defaultWidth)
