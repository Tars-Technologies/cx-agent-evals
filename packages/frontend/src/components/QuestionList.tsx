...
export function QuestionList({
  ...
  const filteredQuestions = questions
    ...
    .filter(({ question }) => {
      ...
      if (sourceFilter === "real-world" && question.source !== "real-world")
        return false
      if (sourceFilter === "generated" && question.source === "real-world")
        return false
      return true
    })
      ...
      {generating && phaseStatus && questions.length > 0 && (
        <div className="px-3 py-2 border-b border-accent/20 bg-accent/5">
          <div className="flex items-center gap-2">
            <span className="w-1 h-1 rounded-full bg-accent animate-pulse-dot" />
            <span className="text-[10px] text-accent/80">{phaseStatus}</span>
          </div>
        </div>
      )}
        ...
        {filteredQuestions.map(({ question, originalIndex }) => {
          ...
          return (
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") onSelect(originalIndex)
              }}
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {question.relevantSpans &&
                  question.relevantSpans.length > 0 && (
                    <span className="text-[10px] text-text-dim">
                      {question.relevantSpans.length} span
                      {question.relevantSpans.length !== 1 ? "s" : ""}
                    </span>
                  )}
                {docCount > 1 && (
