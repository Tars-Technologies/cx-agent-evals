...
export function SimRunDetail({ runId }: { runId: Id<"conversationSimRuns"> }) {
        ...
        <div className="flex-1 min-w-0 overflow-y-auto">
            ...
            {groupMessagesWithToolCalls(messages).map((item) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`text-[9px] mb-0.5 ${isUser ? "text-white/50" : "text-text-dim"}`}
                    >
                      {isUser ? "User" : "Agent"}
                    </div>
                    {msg.content}
          ...
          {run.evaluatorResults && run.evaluatorResults.length > 0 && (
            <div className="px-4 py-3 border-t border-border">
              <h3 className="text-[11px] text-text-dim uppercase tracking-wider mb-2">
                Evaluation (
                {run.evaluatorResults.filter((r) => r.passed).length}/
                {run.evaluatorResults.length} passed)
              </h3>
                ...
                {run.evaluatorResults.map((result, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[10px] font-medium ${result.passed ? "text-green-400" : "text-red-400"}`}
                      >
