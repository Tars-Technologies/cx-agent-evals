...
export function SimScenarioList({
  ...
  const runs =
    useQuery(api.conversationSim.runs.bySimulation, { simulationId }) ?? []
  ...
  const startEvaluation = useMutation(
    api.conversationSim.orchestration.startEvaluation
  )
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {phase === "evaluation" &&
        (!simulation?.evaluationStatus ||
          simulation.evaluationStatus === "not_started") ? (
          <div className="p-4 text-center space-y-3 mt-8">
            <p className="text-xs text-text-dim">
              Conversations complete. Run evaluation to score them.
            </p>
            <button
          ...
          [...grouped.entries()].map(
            ([scenarioId, scenarioRuns], scenarioIndex) => {
              const allConversationsComplete = scenarioRuns.every(
              ...
              const evaluationDone =
                simulation?.evaluationStatus === "completed"
              const isSelected = scenarioRuns.some(
                (r) => r._id === selectedRunId
              )
                ...
                >
                  ...
                  <div className="px-3 py-2">
                    <div className="flex items-center justify-between">
                      ...
                      {phase === "conversations" ? (
                        <span
                          className={`flex-shrink-0 ml-2 text-[10px] font-medium px-1.5 py-0.5 rounded ${
      ...
      {showEvalModal && (
        <div
          ...
          onClick={() => !startingEval && setShowEvalModal(false)}
        >
          <div
            className="bg-bg-elevated border border-border rounded-lg shadow-xl w-full max-w-md p-6"
          ...
          >
            ...
            <p className="text-xs text-text-dim mb-4">
              Pick an evaluator set to score the {grouped.size} scenarios in
              this simulation.
            </p>
            ...
            <select
              ...
              onChange={(e) =>
                setSelectedEvalSetId(e.target.value as Id<"evaluatorSets">)
              }
