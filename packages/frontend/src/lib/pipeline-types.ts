...
const registryPresets = Object.fromEntries(
  PRESET_REGISTRY.filter((p) => p.status === "available").map((p) => [
    p.id,
    p.config as PipelineConfig
  ])
)
...
export const PRESET_NAMES = PRESET_REGISTRY.filter(
    ...
    index:
      strategy === "parent-child"
        ? {
            ...
            chunkOverlap: 0,
            embeddingModel:
              index.embeddingModel ?? DEFAULT_INDEX_CONFIG.embeddingModel!,
            childChunkSize:
              (index as ParentChildIndexConfig).childChunkSize ?? 200,
            parentChunkSize:
              (index as ParentChildIndexConfig).parentChunkSize ?? 1000,
            childOverlap: (index as ParentChildIndexConfig).childOverlap ?? 0,
            parentOverlap:
              (index as ParentChildIndexConfig).parentOverlap ?? 100
          }
        : {
            strategy,
            chunkSize:
              (index as PlainIndexConfig).chunkSize ??
              DEFAULT_INDEX_CONFIG.chunkSize!,
            chunkOverlap:
              (index as PlainIndexConfig).chunkOverlap ??
              DEFAULT_INDEX_CONFIG.chunkOverlap!,
            embeddingModel:
              index.embeddingModel ?? DEFAULT_INDEX_CONFIG.embeddingModel!,
            ...((index as PlainIndexConfig).separators
              ? { separators: (index as PlainIndexConfig).separators }
              : {})
          },
