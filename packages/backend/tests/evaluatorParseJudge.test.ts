...
describe("parseJudgeResponse", () => {
  describe("well-formed JSON", () => {
    ...
    it("accepts yes/no as aliases for pass/fail", () => {
      expect(
        parseJudgeResponse(JSON.stringify({ answer: "yes" })).verdict
      ).toBe("pass")
    ...
    it("is case-insensitive on the answer value", () => {
      expect(
        parseJudgeResponse(JSON.stringify({ answer: "PASS" })).verdict
      ).toBe("pass")
      expect(
        parseJudgeResponse(JSON.stringify({ answer: "Fail " })).verdict
      ).toBe("fail")
    })
  })
...
\`\`\``
  ...
  describe("error cases (previously silently mislabeled)", () => {
    ...
    it("throws when JSON parses but is not an object", () => {
      expect(() => parseJudgeResponse("42")).toThrow(
        /Unparseable judge response/
      )
