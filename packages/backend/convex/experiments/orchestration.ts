  ...
  handler: async (ctx, { context, result }: {
    ...
    if (result.kind === "canceled") {
      await ctx.db.patch(context.experimentId, {
        status: "canceled",
        // Stamp completedAt only on the first terminal transition so a later
        // terminal write can't overwrite the true finish time.
    ...
    // result.kind === "failed"
    // A late WorkPool failure (e.g. timeout) can arrive after the action has
    // already persisted a terminal status; preserve that source of truth
    // rather than overwriting a completed/canceled experiment with "failed".
