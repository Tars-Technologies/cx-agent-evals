...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (dataset.kbId !== args.kbId)
      throw new Error("Dataset does not belong to this KB")
    ...
    for (const retrieverId of args.retrieverIds) {
      ...
      if (retriever.kbId !== args.kbId)
        throw new Error(
          `Retriever ${retriever.name} does not belong to this KB`
        )
      if (retriever.status !== "ready")
        throw new Error(
          `Retriever ${retriever.name} is not ready (status: ${retriever.status})`
        )
    }
  ...
  handler: async (ctx, args) => {
    ...
    const results = await Promise.all(
      children.map(async (exp) => {
        const retriever = exp.retrieverId
          ? await ctx.db.get(exp.retrieverId)
          : null
        const scores = (exp.scores ?? {}) as Record<string, number>
        const recall = scores.recall ?? 0
        const precision = scores.precision ?? 0
        const compositeScore =
          run.scoringWeights.recall * recall +
          run.scoringWeights.precision * precision
        ...
        return {
          ...
          totalQuestions: exp.totalQuestions ?? null,
          processedQuestions: exp.processedQuestions ?? null,
          failedQuestions: exp.failedQuestions ?? null,
          recall,
          precision,
          f1: scores.f1,
          iou: scores.iou,
...
export const onChildComplete = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const exp of children) {
      if (exp.status !== "completed" && exp.status !== "completed_with_errors")
        continue
      const scores = (exp.scores ?? {}) as Record<string, number>
      const recall = scores.recall ?? 0
      const precision = scores.precision ?? 0
