...
export const backfillBehaviorAnchors = internalAction({
  ...
  handler: async (
    ...
    const page = await ctx.runQuery(
      internal.conversationSim.migrations.pageScenarios,
      {
        cursor: cursor ?? null,
      ...
      try {
        ...
        if (Array.isArray(anchors)) {
          await ctx.runMutation(
            internal.conversationSim.migrations.patchBehaviorAnchors,
            {
              id: s._id,
  ...
  handler: async (
    ...
    const orgPools = new Map<
      string,
      {
        transcripts: any[]
        exemplars: any[]
        stats?: { median: number; p90: number }
      }
    >()
    ...
    for (const s of page.page) {
      ...
      if (!pool) {
        const transcripts = await ctx.runQuery(
          internal.conversationSim.migrations.listOrgTranscripts,
          {
            orgId: s.orgId,
            limit: 50
          }
      ...
      if (pool.exemplars.length === 0) {
        ...
        console.warn(
          `backfillSynthetic: no transcripts available for org ${s.orgId}; skipping ${s._id}`
        )
        continue
      }
        ...
        const transcriptText = pool.exemplars
          .flatMap((ex: any) =>
            ex.messages
              .filter((m: any) => m.role === "human_agent" || m.role === "user")
              .map(
                (m: any) => `${m.role === "user" ? "user" : "agent"}: ${m.text}`
              )
          )
          .join("\n")
        const prompt = `Persona: ${s.persona.type} (${s.persona.traits.join(", ")}, ${s.persona.communicationStyle}, ${s.persona.patienceLevel} patience)
...
${BEHAVIOR_ANCHORS_INSTRUCTION}`
        ...
        console.error(
          `backfillSynthetic anchor generation failed for ${s._id}:`,
          e
        )
      ...
      await ctx.runMutation(
        internal.conversationSim.migrations.patchSyntheticBackfill,
        {
          id: s._id,
