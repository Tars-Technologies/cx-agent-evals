...
function deduplicateChunks(
  ...
  const result: Array<{
    id: ReturnType<typeof PositionAwareChunkId>
    content: string
    docId: ReturnType<typeof DocumentId>
    start: number
    end: number
    metadata: Record<string, unknown>
  }> = []
...
async function processQueryForRetriever(
  ...
  switch (strategy) {
    ...
    case "multi-query": {
      ...
      const prompt = (
        (queryConfig.generationPrompt as string) ?? DEFAULT_MULTI_QUERY_PROMPT
      ).replace("{n}", String(n))
    ...
    case "step-back": {
      if (!llm) return [query]
      const prompt =
        (queryConfig.stepBackPrompt as string) ?? DEFAULT_STEP_BACK_PROMPT
      const abstract = await llm.complete(prompt + query)
      return queryConfig.includeOriginal !== false
        ? [query, abstract]
        : [abstract]
    }
    case "rewrite": {
      if (!llm) return [query]
      const prompt =
        (queryConfig.rewritePrompt as string) ?? DEFAULT_REWRITE_PROMPT
  ...
  for (const step of steps) {
    switch (step.type as string) {
      ...
      case "dedup":
        current = applyDedup(
          current,
          (step.method as "exact" | "overlap") ?? "overlap",
          (step.overlapThreshold as number) ?? 0.5
        )
        break
      case "mmr":
...
export const retrieve = tenantAction({
  ...
  > => {
    ...
    const refinementSteps = Array.isArray(configObj.refinement)
      ? (configObj.refinement as Array<Record<string, any>>)
      : []
    ...
    const llm = needsLLM
      ? await OpenAIPipelineLLM.create({ model: "gpt-4o-mini" })
      : undefined
    ...
    if (searchStrategy === "bm25") {
      const rawChunks = await loadAllChunksForRetriever(
        ctx,
        retriever.kbId,
      ...
      const candidateMultiplier =
        (searchSettings.candidateMultiplier as number) ?? 4
      ...
      doSearch = async (q, k) => {
        ...
        const fusionMethod =
          (searchSettings.fusionMethod as string) ?? "weighted"
        const fused =
          fusionMethod === "rrf"
            ? reciprocalRankFusion({
                denseResults,
                sparseResults,
                k: searchSettings.rrfK as number | undefined
              })
            : weightedScoreFusion({
                denseResults,
                sparseResults,
    ...
    try {
      ...
      const queries = await processQueryForRetriever(
        args.query,
        queryConfigObj,
        llm
      )
