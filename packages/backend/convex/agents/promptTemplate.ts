...
export function composeSystemPrompt(
  ...
  if (agent.identity.companyContext) {
    identityLines.push(
      "",
      `## About ${agent.identity.companyName}`,
  ...
  if (agent.responseStyle.formality)
    styleLines.push(`Formality: ${agent.responseStyle.formality}`)
  if (agent.responseStyle.length)
    styleLines.push(`Response length: ${agent.responseStyle.length}`)
  if (agent.responseStyle.formatting)
    styleLines.push(`Formatting: ${agent.responseStyle.formatting}`)
  if (agent.responseStyle.language)
    styleLines.push(`Always respond in: ${agent.responseStyle.language}`)
